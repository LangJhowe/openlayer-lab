  ...
  constructor(opts) {
    ...
    ele.addEventListener('longpress', () => {
      ...
      let pixel = map.getPixelFromCoordinate(this.position),
        TRBL = map.getViewport().getBoundingClientRect()
      let originalEvent = new PointerEvent('pointermove', {
        pointerId: Date.now(),
        bubbles: true,
        cancelable: true,
        pointerType: 'mouse',
        width: 1,
        height: 1,
        clientX: TRBL.left + pixel[0],
        clientY: TRBL.top + pixel[1],
        // pageX: 5,
        // pageY: 2,
        isPrimary: true,
        composed: true,
      ...
      let modify = this.draw.modify
      let mapBrowserEvent = new MapBrowserEvent('pointerdrag', map, originalEvent, true)
      mapBrowserEvent.pixel = pixel
      mapBrowserEvent.coordinate = this.position
      mapBrowserEvent.target = map
      // console.log(map.getEventPixel(originalEvent))
      let event = new ModifyEvent('modifystart', new Collection([this.shape.feature]), mapBrowserEvent)
      modify.dispatchEvent(event)
    })
          ...
          if (this.type == GeometryType.LINE_STRING || this.type == GeometryType.POLYGON) {
            ...
            let pos =
                this.type == GeometryType.LINE_STRING
                  ? geo.getLastCoordinate()
                  : geo.getInteriorPoint().getCoordinates(),
            ...
            if (draw.drawing) {
              if (this.type == GeometryType.POLYGON && this.nodes.length < 2) {
                data.tipsTemplate = '<div class="ol-measure-label__tooltip">单击确定地点;右键取消</div>'
              } else if (this.type == GeometryType.LINE_STRING && this.nodes.length < 1) {
                //
              } else {
                data.tipsTemplate = '<div class="ol-measure-label__tooltip">单击确定地点,双击结束;右键取消</div>'
              }
            } else {
              data.tipsTemplate = ''
            }
            if (!draw.outsideViewportDrawing) {
...
export { MeasureLine, MeasurePolygon }
